  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      nodes.forEach((a, i) => {
        ...
        if (best < 0) return;
        ctx.strokeStyle = "rgba(94,70,191,0.14)";
        ctx.setLineDash([3, 5]);
        ctx.beginPath();
      ...
      nodes.forEach((a, i) => {
        ...
        nodes.forEach((b, j) => {
          if (j <= i || !b.lit) return;
          ctx.strokeStyle = "rgba(94,70,191,0.20)";
          ctx.beginPath();
          ctx.moveTo(pts[i].x, pts[i].y);
      ...
      nodes.forEach((node, i) => {
        ...
        if (node.lit) {
          ...
          glow.addColorStop(0, "rgba(116,98,213,0.35)");
          glow.addColorStop(1, "rgba(116,98,213,0)");
          ctx.fillStyle = glow;
          ctx.beginPath();
