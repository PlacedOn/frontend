...
const TONE_DEEP = [139, 84, 255] as const; // #8E64FF
const TONE_PALE = [183, 155, 255] as const; // #AB95FF
...
export function CalmField({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      grad.addColorStop(0, `rgba(142, 100, 255,${(0.2 + 0.13 * b).toFixed(3)})`);
      grad.addColorStop(0.55, `rgba(142, 100, 255,${(0.1 + 0.08 * b).toFixed(3)})`);
      ...
      if (b > 0.004) {
        ctx.lineWidth = 1.75;
        ctx.strokeStyle = "rgba(142, 100, 255,0.55)";
        ctx.beginPath();
